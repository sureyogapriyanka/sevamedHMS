...
exports.updateQueueEntry = async (req, res) => {
    try {
        ...
        } else if (status !== 'completed' && queueEntry.status === 'completed') {
            // If status is being changed from completed back to another status,
            // move all entries at or after this position down by one
            await Queue.updateMany(
                {
                    doctorId: queueEntry.doctorId,
                    position: { $gte: queueEntry.position }
                },
                { $inc: { position: 1 } }
            );
        }
